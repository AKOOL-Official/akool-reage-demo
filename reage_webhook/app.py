...
@app.route('/reage-app', methods=['GET'])
def reage_app():
    #emit socket event
    ...
    try:
        ...
        try:
            ...
            if decrypted_json['status'] == 3:
                ...
                print("Reage process failed")
                socketio.emit('message', {
                    'type': 'error',
                    'message': 'Reage processing failed. This could be due to invalid video/image format, network issues, or processing errors. Please try again or contact support if the issue persists.'
                })
            else:
